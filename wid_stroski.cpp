...
void wid_stroski::napolni() {
    ...
    if ( ui->tbl_stroski->selectedItems().count() > 0 ) {
        izbranec = ui->tbl_stroski->selectedItems().takeAt(0)->row();
    }

    razvrsti = ui->tbl_stroski->horizontalHeader()->sortIndicatorSection();
    ...
    ui->tbl_stroski->selectRow(izbranec);
    ui->tbl_stroski->sortByColumn(razvrsti, Qt::AscendingOrder);
