...
void wid_izdatki::napolni_leto() {
    ...
    if ( ui->tbl_letni_pregled->selectedItems().count() > 0 ) {
        izbranec = ui->tbl_letni_pregled->selectedItems().takeAt(0)->row();
    }

    razvrsti = ui->tbl_letni_pregled->horizontalHeader()->sortIndicatorSection();
    ...
    ui->tbl_letni_pregled->selectRow(izbranec);
    ui->tbl_letni_pregled->sortByColumn(razvrsti, Qt::AscendingOrder);
